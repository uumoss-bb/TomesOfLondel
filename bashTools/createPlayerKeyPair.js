...
async function main() {
  ...
  const accessKey = makePlayerAccessKey({
    owner
  })

  let secret = accessKey.secret

  const superSecret = await GetSecrects("WEBGLPLAYER_SECRET")
...
function makePlayerAccessKey(data) {
  let PlayerAccessData = {
    name: data.owner,
    enabled: true,
    features: {
      showTitleBar: true,
      ...
      hideHelpDocument: true,
      developSupported: false,
      presentationModeIsSupported: false,
      customCacheSupported: false,
      theaterDisplaySupported: false,
      canAuthor: false,
      allowDevelopmentTools: false,
      screenshotSupported: false,
      theaterModeSupported: false,
      presentationModeSupported: false,
      customCacheIsSupported: false,
      narration: false,
      allowZSpace: false,
    }
  }
  ...
  let newData = {
    ...
    access_id: uuid.v4(),
    secret: uuid.v4(),
    data: PlayerAccessData,
    type: "player-accessKey"
  }
